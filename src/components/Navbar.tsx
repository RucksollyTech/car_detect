  ...
  return (
    <nav className="border-b border-gray-800 bg-gray-950 px-6 py-4 flex items-center justify-between">
      ...
      <div className="flex items-center gap-6">
        <Link href="/dashboard" className="text-sm text-gray-400 hover:text-white transition hidden md:inline">Detect</Link>
        <Link href="/dashboard/history" className="text-sm text-gray-400 hover:text-white transition hidden md:inline">History</Link>
        <div className="flex items-center gap-3">
          <span className="text-sm text-gray-500">
            <TruncateText text={user?.username || ''} />
          </span>
          <button onClick={logout} className="text-sm hidden md:inline cursor-pointer text-gray-400 hover:text-red-400 transition">
            Sign out
          </button>
        </div>
      </div>
